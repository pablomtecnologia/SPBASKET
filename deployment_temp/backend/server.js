...
const pool = new Pool({
    ...
    // ssl: { rejectUnauthorized: false } // REMOVED for local VPS connection
});
...
app.post('/api/visits', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Logged anyway (error suppressed)' });
    }
});

app.get('/api/visits/count', async (req, res) => {
    try {
        // Contamos todas las visitas o visitas únicas por IP? 
        // El usuario dijo "numero sin mas", así que total COUNT es lo más impresionante.
        const { rows } = await pool.query('SELECT COUNT(*) as total FROM site_visits');
        res.json({ total: parseInt(rows[0].total) });
    } catch (err) {
        res.status(500).json({ message: 'Error counting visits' });
    }
});
...
(async () => {
    try {
        ...
        try {
            ...
            await client.query("INSERT INTO system_settings (key, value) VALUES ('mvp_voting_open', FALSE) ON CONFLICT DO NOTHING");
            await client.query("INSERT INTO system_settings (key, value) VALUES ('quiniela_open', FALSE) ON CONFLICT DO NOTHING");
            console.log('✅ Tabla system_settings verificada');
        } finally { client.release(); }
    } catch (err) { console.warn('⚠️ Error init system_settings:', err.message); }
})();
...
app.put('/api/settings', verifyToken, async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ message: 'Error updating setting' }); }
});

// Estadísticas detalladas para admin
app.get('/api/visits/stats', verifyToken, async (req, res) => {
    if (req.user.rol !== 'admin') return res.status(403).json({ message: 'Forbidden' });
    try {
        const { rows } = await pool.query(`
