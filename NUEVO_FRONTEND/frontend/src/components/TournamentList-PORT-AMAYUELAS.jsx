...
const getTournamentLastDate = (tournament) => {
  const jornadaDates = (tournament?.jornadas || [])
    .map(jornada => String(jornada?.date || '').trim())
    .filter(Boolean)
    .sort()

  if (jornadaDates.length > 0) return jornadaDates[jornadaDates.length - 1]
  return String(tournament?.date || '').trim()
}
...
export default function TournamentList({ onSelect, selected, extraActions = null, canCreateTournaments = true }) {
  ...
  const [showExpiredTournaments, setShowExpiredTournaments] = useState(false)
  ...
  const visibleTournaments = tournaments.filter(tournament => {
    if (showExpiredTournaments) return true
    const lastDate = getTournamentLastDate(tournament)
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'flex-start' }}>
        ...
        {extraActions}
        <label className="form-input" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: 0, cursor: 'pointer' }}>
          <input
            type="checkbox"
            checked={showExpiredTournaments}
            onChange={e => setShowExpiredTournaments(e.target.checked)}
          />
          <span>Torneos vencidos</span>
        </label>
        </div>
      </div>
