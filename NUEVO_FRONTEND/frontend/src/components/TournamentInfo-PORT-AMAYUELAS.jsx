...
import { updateTournament, uploadImage } from '../api'
...
export default function TournamentInfo({ tournament, onUpdate }) {
  ...
  const renderPosterSection = ({ icon, label, field, placeholder }) => {
    const currentImageUrl = previews[field] || form[field]
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(480px, 1fr))', gap: '0', alignItems: 'start' }}>
        <div style={{ paddingRight: '0.75rem' }}>
          ...
          <div className="card" style={{ marginBottom: '1.5rem', border: '1px solid rgba(249,115,22,0.35)', background: 'rgba(249,115,22,0.06)' }}>
            <div className="card-title" style={{ fontSize: '1.1rem', marginBottom: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
              {privateUrls.map(item => (
                <div key={item.label}>
                  ...
                  <a href={item.url} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)', wordBreak: 'break-all' }}>
                    {item.url}
                  </a>
                </div>
              ))}
