...
const { initMatches, allocateSchedules, splitIntoGroups } = require('./services/scheduler');
const multer = require('multer');
const path = require('path');
const fs = require('fs');

const app = express();
const DEFAULT_GROUP_LOGIC_NAME = 'LOGICA SPBASKET (POR DEFECTO)'
const DEFAULT_GROUP_LOGIC_CONFIG = JSON.stringify({
  definitions: [
    ...
    },
    {
      ...
      positions: ['A1', '', 'C3', 'B3', 'B2', 'A4', '', 'A2', 'C1', '', 'C2', 'B4', 'A3', 'C4', '', 'B1']
    },
    {
      teamCount: 13,
      roundTripCount: 1,
...
async function ensureDefaultGroupLogic() {
  ...
  await prisma.tournament.updateMany({
    where: { groupLogicId: null },
    data: { groupLogicId: existing.id }
  })
...
app.post('/api/matches/:id/join', async (req, res) => {
  ...
  try {
    ...
    const official = await prisma.official.findFirst({
      where: {
        ...
        OR: [
          ...
          { lastName: officialName }
        ]
      }
    });

    let joinedOfficial = official;

    if (!official) {
      const [firstName = '', ...lastNameParts] = String(officialName || '').trim().split(' ');
      const normalizedLastName = lastNameParts.join(' ').trim();
      const fullNameOfficial = await prisma.official.findFirst({
        where: {
          tournamentId: match.category.tournamentId,
          isOperational: true,
          firstName,
      ...
      joinedOfficial = fullNameOfficial;
    } else {
      joinedOfficial = await prisma.official.findUnique({
    ...
    if (match.isLive && match.officialName) {
      if (match.officialName !== officialName) {
    ...
    const updated = await prisma.match.update({
      where: { id: matchId },
      data: { officialName, isLive: true, liveSessionKey: sessionKey }
    });

    // Registramos el log de entrada
    await createLog(matchId, officialName, "JOIN", updated, "Entrada al acta digital");
...
app.put('/api/tournaments/:id', async (req, res) => {
  try {
    ...
    const currentTournament = await prisma.tournament.findUnique({
      where: { id: tournamentId },
      select: { assignOfficialsToCourt: true }
    });
...
app.post('/api/tournaments/:tid/jornadas', async (req, res) => {
  try {
    ...
    const jornada = await prisma.jornada.create({
      data: { date, startTime, endTime, tournamentId: parseInt(req.params.tid) },
    });
...
app.post('/api/tournaments/:tid/categories', async (req, res) => {
  try {
    const { name, color, minAge, maxAge, gender, isVeteran } = req.body;
    if (!name) return res.status(400).json({ error: 'El nombre es obligatorio' });
...
app.put('/api/teams/:id', async (req, res) => {
  try {
    ...
    const { name, club, contactName, contactLastName, contactPhone, contactEmail } = req.body;
    const team = await prisma.team.update({
      where: { id: parseInt(req.params.id) },
      data: { name, club, contactName, contactLastName, contactPhone, contactEmail },
      include: { players: true },
    });
    res.json(team);
  } catch (e) {
...
async function generateFinalPhaseBrackets(categoryId) {
  ...
  // Helper: mapear nombre de fase a etiqueta de observaciones
  const getPhaseLabel = (groupName) => {
...
app.post('/api/tournaments/:tid/categories/:cid/generate-matches', async (req, res) => {
  try {
    ...
    const matches = initMatches(groups, categoryId);
...
app.patch('/api/schedule/:id', async (req, res) => {
  try {
    ...
    const tournament = await prisma.tournament.findUnique({ where: { id: existingSlot.tournamentId } });
    ...
    if (hId || aId) {
      ...
      for (const tm of teamMatches) {
        if (tm.date === targetDate) {
          ...
          if (diff < (matchDuration * 2)) {
            return res.status(400).json({ error: `Falta descanso: Los equipos deben tener al menos una franja de descanso entre partidos.` });
          }
        }
      }
    }
