...
app.get('/api/admin/fecan/matches/:teamId', verifyToken, async (req, res) => {
    ...
    const fecanId = 4046; // ID en FECAN (ajustar segÃºn equipo si es necesario)

    try {
        console.log(`ðŸ” Vista previa FECAN para ${teamId} (ID FECAN: ${fecanId})`);
        ...
        console.error('âŒ Error obteniendo vista previa FECAN:', error);
...
app.post('/api/admin/fecan/import/:teamId', verifyToken, async (req, res) => {
    ...
    try {
        console.log(`ðŸ“¥ Importando partidos FECAN para ${teamId}...`);
        ...
        console.error('âŒ Error importando FECAN:', error);
        res.status(500).json({ message: 'Error durante la importaciÃ³n: ' + error.message });
    }
});
    ...
    `, [teamId]);
        ...
        console.error('âŒ Error obteniendo partidos de FECAN:', error);
        res.status(500).json({ message: 'Error obteniendo partidos' });
    }
});

// GET /api/admin/fecan/logs - Ver logs de importaciÃ³n (solo admin)
...
SELECT * FROM fecan_import_logs 
    ...
    `);
        ...
        console.error('âŒ Error obteniendo logs FECAN:', error);
        res.status(500).json({ message: 'Error obteniendo logs' });
    }
});

// POST /api/admin/fecan/run-now - Ejecutar importaciÃ³n manual inmediatamente (solo admin)
app.post('/api/admin/fecan/run-now', verifyToken, async (req, res) => {
    if (req.user.rol !== 'admin') return res.status(403).json({ message: 'Admin requerido' });

    try {
        console.log('ðŸš€ Ejecutando importaciÃ³n manual FECAN...');
        const results = await runAutoImport(pool);

        res.json({
            message: 'ImportaciÃ³n manual completada',
            results: results
        });
    } catch (error) {
        console.error('âŒ Error en importaciÃ³n manual FECAN:', error);
        res.status(500).json({ message: 'Error en importaciÃ³n manual: ' + error.message });
    }
});

// ========== CRON JOB: IMPORTACIÃ“N AUTOMÃTICA CADA 2 HORAS ==========
// Se ejecuta cada 2 horas: 00:00, 02:00, 04:00, 06:00, 08:00, 10:00, 12:00, 14:00, 16:00, 18:00, 20:00, 22:00
cron.schedule('0 */2 * * *', async () => {
    console.log('\nðŸ¤– â° Ejecutando importaciÃ³n automÃ¡tica FECAN (cada 2 horas)...');
    try {
        const results = await runAutoImport(pool);
        console.log('âœ… ImportaciÃ³n automÃ¡tica completada:', results);
    } catch (error) {
        console.error('âŒ Error en importaciÃ³n automÃ¡tica FECAN:', error);
    }
}, {
    scheduled: true,
...
console.log('ðŸ¤– Cron job de FECAN configurado: cada 2 horas');

// Ejecutar importaciÃ³n inicial al arrancar el servidor (opcional)
// Comentar esta lÃ­nea si NO quieres que importe al iniciar el servidor
setTimeout(async () => {
    console.log('ðŸš€ Ejecutando importaciÃ³n inicial de FECAN al arrancar servidor...');
    try {
        await runAutoImport(pool);
        console.log('âœ… ImportaciÃ³n inicial completada');
    } catch (error) {
        console.error('âš ï¸ Error en importaciÃ³n inicial:', error);
    }
}, 5000);
