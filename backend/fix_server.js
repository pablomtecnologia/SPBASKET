...
// Leer server.js original. Asumimos UTF-8 ya que lo convertí antes.
const original = fs.readFileSync('server.js', 'utf8');

// Buscar punto de corte
// El comentario puede tener espacios variables o encoding issues. Busquemos substring segura.
// "5000); // Esperar 5 segundos"
const keyword = '5000); // Esperar 5 segundos';
const index = original.lastIndexOf(keyword);

if (index === -1) {
    console.error('❌ NO SE ENCONTRÓ EL PUNTO DE TRUNCADO (last index)');
    // Intentar buscar líneas anteriores
    const altKey = 'cron.schedule';
    const idx2 = original.lastIndexOf(altKey);
    if (idx2 !== -1) {
        console.log('⚠️ Fallback a cron.schedule...');
        // ... Lógica compleja, mejor abortar.
    }
    process.exit(1);
}

// Cortar justo después de la linea completa (hasta el \n siguiente)
const newlineIndex = original.indexOf('\n', index);
const cutPoint = newlineIndex !== -1 ? newlineIndex + 1 : index + keyword.length;

const cleanPart1 = original.substring(0, cutPoint);

// Unir
const finalContent = cleanPart1 + '\n' + contentNew;

// Escribir server_repaired.js
fs.writeFileSync('server_repaired.js', finalContent, 'utf8');
console.log('✅ server_repaired.js generado correctamente');

// Reemplazar server.js
try {
    fs.renameSync('server_repaired.js', 'server.js'); // Esto puede fallar en Windows si busy
} catch (e) {
    // Si falla, hacemos copia
    fs.copyFileSync('server_repaired.js', 'server.js');
}
console.log('✅ server.js FINALIZADO');
